...
description: Lär dig felsöka en Adobe Target-implementering med Adobe Experience Platform Web SDK. Här finns felsökningsalternativ, webbläsartillägg och skillnader mellan at.js och Platform Web SDK.
...
| Funktion eller verktyg | support för at.js | Stöd för SDK |
| --- | --- | --- |
...
## Adobe Experience Platform Debugger webbläsartillägg
...
| at.js, funktion | SDK-motsvarighet för plattformar |
| --- | --- |
| **Mbox Inaktivera** - Inaktivera mål från hämtning och återgivning för att kontrollera om sidan har brutits utan målinteraktioner<br><br>Läs in sida med URL-parameter: `mboxDisable=true` | Ingen direkt motsvarighet. Du kan blockera alla förfrågningar från Platform Web SDK med utvecklarverktygen i din webbläsare. |
| **Mbox Debug** - loggar alla at.js-åtgärder i webbläsarens konsol för att felsöka återgivningsproblem<br><br>Läs in sida med URL-parameter: `mboxDebug=true` | **Tillåt felsökning** - loggar detaljerade åtgärder för SDK, inklusive men inte begränsat till Target-personaliseringsåtgärder.<br><br>Läs in sida med URL-parameter: `alloy_debug=true` <br /><br />Eller kör `alloy("setDebug", { "enabled": true });` i utvecklarkonsolen |
...
1. Högerklicka någonstans på sidan och välj **Inspektera**
1. Välj fliken **Nätverk**
1. Filtrera efter strängen `//ee//` så att endast SDK-anrop för plattformen visas
1. Läs in sidan igen
...
| Loggad information | at.js (`mboxDebug=true`) | Plattform, SDK (`alloy_debug=true`) |
| --- | --- | --- |
...
>[!NOTE]
>
>Felsökningsloggar för at.js och Platform Web SDK ger en liknande detaljnivå med det anmärkningsvärda undantaget att Web SDK endast meddelar om återgivningsfel på grund av ogiltiga väljare. Felsökningsloggningen bekräftar för närvarande inte att återgivningen lyckades.
...
| Adobe Experience Platform Debugger webbläsartillägg | Ja | Ja |
...
1. Öppna Adobe Experience Platform Debugger-tillägget genom att markera ikonen (![](assets/start-icon.jpg)) i webbläsarens navigeringsfält
...
1. Öppna Adobe Experience Platform Debugger webbläsartillägg och anslut en fjärrfelsökningssession enligt ovan
...
![Målspecifika element för SDK-nyttolasten för Platform Web](assets/target-payload.png){zoomable="yes"}
...
![Målspecifika element i SDK-svarstexten för plattformen](assets/target-response.png){zoomable="yes"}

- Platform Web SDK returnerar åtgärder för alla Adobe-program under objektet `handle`
- Åtgärden `personalization:decisions` innebär ett svar från Target eller erbjudandebeslut
