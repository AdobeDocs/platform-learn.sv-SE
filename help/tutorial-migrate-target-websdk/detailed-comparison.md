...
| `bodyHidingEnabled` | Om en `prehidingStyle` är definierad med `configure` och den här funktionen aktiveras. Om inget format definieras försöker inte Platform Web SDK att dölja något innehåll. |
...
| `deviceIdLifetime` | Stöds inte. If `targetMigrationEnabled` är inställd på `true` med `configure` kommando, `mbox` cookie anges med enhetens livstid inställd på 2 år. Detta värde kan inte konfigureras. |
