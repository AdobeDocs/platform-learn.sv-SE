...
>
>* Se de fullständiga kraven för [Integrering av bastjänster](https://experienceleague.adobe.com/docs/core-services/interface/about-core-services/core-services.html)
>* Se de fullständiga kraven för [Analyser för Target-integrering](https://experienceleague.adobe.com/docs/target/using/integrate/a4t/before-implement.html)
>* ha en administratör för din Experience Cloud-organisation [begära etablering av dessa integreringar](https://www.adobe.com/go/audiences)


## Målgrupper

[Målgrupper](https://experienceleague.adobe.com/docs/core-services/interface/audiences/audience-library.html) är en del av Bastjänsten för människor och gör det möjligt att dela målgrupper mellan olika lösningar. Du kan till exempel skapa en målgrupp i Audience Manager och använda den för att leverera personaliserat innehåll med Target.
...
1. Klicka **[!UICONTROL Rensa alla begäranden]** bara för att snygga till saker
...
1. Titta på raden med namnet&quot;Experience Cloud Visitor ID&quot;. ID:n i varje begäran från varje lösning ska alltid vara desamma.
   ...
   ![Bekräfta matchande SDID:n](images/integrations-matchingECIDs.png)

1. ID:n är unika per besökare, som du kan bekräfta genom att be en medarbetare att upprepa dessa steg.

## Analyser för mål (A4T)

The [Analyser för mål (A4T)](https://experienceleague.adobe.com/docs/target/using/integrate/a4t/a4t.html) Tack vare integreringen kan ni utnyttja era Analytics-data som källa för att rapportera mätvärden i Target.
...
1. Titta på raden med namnet&quot;Kompletterande data-ID&quot;. ID:n från den första sidinläsningen ska matcha mellan Target och Analytics. ID:n från den andra sidinläsningen ska också matcha, men skilja sig från den första sidinläsningen.
   ...
   ![Bekräfta matchande SDID:n](images/integrations-matchingSDIDs.png)
...
[Kundattribut](https://experienceleague.adobe.com/docs/core-services/interface/customer-attributes/attributes.html) är en del av People Core Service som gör att du kan överföra data från CRM-databasen (customer relationship management) och använda dem i Adobe Analytics och Adobe Target.

De viktigaste kraven för att implementera kundattribut - som du redan har gjort - är att:

1. Implementera Adobe Experience Platform Identity Service
1. Ange kund-ID:n via Id-tjänsten *före* Target och Analytics skickar sina förfrågningar (som du har lyckats med hjälp av funktionen för regelordning i taggar)
...
För närvarande är Kund-ID en av de få parametrar som inte visas i Experience Cloud Debugger, så du använder webbläsarens JavaScript-konsol för att visa den.
...
1. Skriv i filterfältet `b/ss` som begränsar vad du ser till Adobe Analytics-förfrågningar
   ...
   ![Öppna Utvecklarverktyg och filtrera fliken Nätverk så att endast Analytics-begäranden visas](images/aam-openTheJSConsole.png)

1. Klicka på **[!UICONTROL INLOGGNING]** länk i webbplatsens övre högra hörn

   ![Klicka på Login (Logga in) längst upp till höger](images/idservice-loginNav.png)
...
1. Bläddra nedåt tills du ser några kapslade parametrar
   ...
   1. id - Kund-ID-värdet kommer från din `Email (Hashed)` dataelement
